...
import { X, Eye, Edit, ExternalLink, Clock, Calendar } from 'lucide-react'
...
export function PreviewSlideout({
  ...
  const entryDescription =
    entry?.fields.description || 'No description available'
          ...
          <div className='flex-shrink-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6'>
            ...
            <div className='flex items-center space-x-4 text-sm text-blue-100'>
              <div className='flex items-center'>
                ...
                {entry?.sys.updatedAt &&
                  formatters.timeAgo(entry.sys.updatedAt)}
              </div>
              <div className='flex items-center'>
                <Clock className='w-4 h-4 mr-1' />
                {entry?.sys.createdAt &&
                  formatters.date(entry.sys.createdAt, 'MMM dd, yyyy')}
              </div>
            </div>
          </div>
                    ...
                    <div className='bg-slate-50 rounded-lg p-4 space-y-2'>
                      ...
                      <div className='flex justify-between'>
                        ...
                        <span className='text-sm'>
                          {formatters.datetime(entry.sys.createdAt)}
                        </span>
                      </div>
                      <div className='flex justify-between'>
                        <span className='text-slate-600'>Updated:</span>
                        <span className='text-sm'>
                          {formatters.datetime(entry.sys.updatedAt)}
                        </span>
                      </div>
                    </div>
                  </div>
